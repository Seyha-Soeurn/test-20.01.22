...
function showNotImportant() {
  ...
  for (let value of tasks) {
    if (value.priority == 0) {
      unImpList.push(value);
    }
  }
  // -2 Call the displayArray with this new list
  displayArray(unImpList)
}
...
function deleteTask(event) {
  ...
  let parent = deleteTarget.parentNode;
  console.log(parent);
  let grParent = parent.parentNode;
  console.log(grParent);
  console.log(grParent.parentNode);
  let index = tasks.findIndex(grParent);
  console.log(index);
